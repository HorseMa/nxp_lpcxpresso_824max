 ...
 */
static TimerHandle_t GpsIconUpdateTimer;

/*!
 * \brief Indicates if the GPS timer is started
 */
static bool GpsIconUpdatexTimerStarted = false;
...
void OnGpsIconUpdateTimerEvent( void )
{
    ...
    // Restart timer
    xTimerStart( &GpsIconUpdateTimer );
}
...
void ScreenDrawGps( uint8_t x, uint8_t y, bool hasFix )
{
    ...
    if( hasFix == true )
    {
        xTimerStop( &GpsIconUpdateTimer );
        GpsIconUpdatexTimerStarted = false;
    ...
    {
        if( GpsIconUpdatexTimerStarted == false )
        {
            GpsIconUpdatexTimerStarted = true;
            GpsIconUpdateState = 0;
            xTimerCreate( &GpsIconUpdateTimer, OnGpsIconUpdateTimerEvent );
            xTimerChangePeriod( &GpsIconUpdateTimer, 1000 );
            xTimerStart( &GpsIconUpdateTimer );
        }
    }
}
